...
pub fn insert_player(conn: &PgConnection, the_group_id: &i32, the_nickname: &str, the_name: &Option<String>) -> QueryResult<String> {
    use crate::schema::players::dsl::*;

    diesel::insert_into(players)
        .values((group_id.eq(the_group_id), nickname.eq(the_nickname), name.eq(the_name)))
        .returning(nickname)
        .get_result(conn)
}
